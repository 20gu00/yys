...
import (
	...
	jobtype2 "go-travel/service/mqueue/cmd/job/jobtype"
...
func (l *CreateHomestayOrderLogic) CreateHomestayOrder(in *pb.CreateHomestayOrderReq) (*pb.CreateHomestayOrderResp, error) {
	...
	order.LiveStartDate = time.Unix(in.LiveStartTime, 0) //time.Unix()函数用于产生本地时间
	order.LiveEndDate = time.Unix(in.LiveEndTime, 0)     //UTC,+in.LiveEndTime second
	...
	payload, err := json.Marshal(jobtype2.DeferCloseHomestayOrderPayload{Sn: order.Sn})
	if err != nil {
		logx.WithContext(l.ctx).Errorf("create defer close order task json Marshal fail err :%+v , sn : %s", err, order.Sn)
	} else {
		_, err = l.svcCtx.AsynqClient.Enqueue(asynq.NewTask(jobtype2.DeferCloseHomestayOrder, payload), asynq.ProcessIn(CloseOrderTimeMinutes*time.Minute))
		if err != nil {
